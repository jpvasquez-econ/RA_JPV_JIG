...
survival_function <- survival_function %>% 
  group_by(buyer, seller) %>% # separating the links between firms
  mutate(link_number = cumsum(c(-1, diff(diff)) < 0)) %>% # if there's interrupted trading between two firms, the new links created, respecting the tolerance, are separated
  ungroup() %>% 
  ...
  filter(!any(yr == 0 & year > 2005) & yr <= 4) %>% # censoring the data: if the observation is greater or equal than year 4 it's not taken into account; the starting links after 2005 are not consider in this exercise
  ungroup() %>% 
  select(yr) %>% 
