...
## unweighted
# this is another way
h <- hist(sellers_per_buyer$n_sellers, main = "Unweighted number of sellers", freq = FALSE)
...
h <- hist(sectors_per_buyer$n_seller_sectors,
  ...
  breaks = seq(
    0, max(sectors_per_buyer$n_seller_sectors) + 1, max(sectors_per_buyer$n_seller_sectors) / 10
  ),
  freq = FALSE
)
...
for (i in as.list(years)) {

  # for each year $t-1$ the set of links between firms is assigned to a dataframe with their respective weights per buyer. Then, the number of sellers per buyer is calcultated.
  temp_df1 <- sales_new %>%
    filter(year == i) %>%
  ...
  # another set of the sellers and buyers in the year $t-1$ and the year $t$ are created.
  temp_df2 <- sales_new %>%
    filter(year == i) %>%
    select(buyer, seller)
  ...
  temp_df2 <- inner_join(temp_df2, temp_df3, by = c("buyer", "seller")) %>%
    count(buyer) # the number of sellers per buyer is computed in the last list

  # the number of sellers per buyer that remained trading with eachother in year $t$ is matched with the number of sellers per buyer in year $t-1$. The missing vlues are changed by 0's
  temp_df1 <- left_join(temp_df1, temp_df2, by = "buyer")

  temp_df1[is.na(temp_df1)] <- 0

  # the observations are matched and then the share that remained is divided by the initial number of sellers per buyer
  temp_df1 <- temp_df1 %>%
  ...
  # the mean is calculated per year weighted and unweighted, then, it's added to the empty dataframe the first time and consecutively per year
  year <- c(i + 1)
