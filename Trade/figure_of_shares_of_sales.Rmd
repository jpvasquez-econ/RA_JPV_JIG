...
alpha_ss <- data.frame(sector_seller = c(NA), sector_buyer = c(NA),
alpha = c(NA)) #3
...
for (i in sectors$sector_seller){ # 4 (a)
  for (j in sectors$sector_seller){
    ...
    sales_states_yr_sectors_ij <- left_join(firms_trade,
      sales_states_yr_sectors_ij) %>% # 7 (b)
    ...
    alpha_ss <- rbind(alpha_ss, c(i, j,
      model$estimate[model$term == "(Intercept)"])) # 10 (b)
  }
}
...
l1_ss_s <- base_ss_s + geom_line(color="#0078b5", size = 0.25) +
ylim(-0.04, 0.04)
l1_ss_s
...
a2_pp <- base_pp + geom_area(fill="#0078b5", alpha=0.75) +
geom_line(color="#0078b5", size = 0.25)
a2_pp

a2_pp_s <- base_pp_s + geom_area(fill="#0078b5", alpha=0.75) +
geom_line(color="#0078b5", size = 0.25)
a2_pp_s

a2_ss_s <- base_ss_s + geom_area(fill="#0078b5", alpha=0.75) +
geom_line(color="#0078b5", size = 0.25)
a2_ss_s
```