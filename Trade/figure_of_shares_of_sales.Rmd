...
```{r share-of-sellers-pp-s, echo=TRUE}
...
for (i in sectors$sector_seller){ # 4 (a)
  for (j in sectors$sector_seller){
    ...
    firms_trade <- expand.grid.df(sector_j_firms, sector_i_firms) %>% #4 (b) and 5
...
l1_pp <- base_pp + geom_line(color="#0078b5", size = 0.25) + ylim(0, 61)
l1_pp

l1_pp_s <- base_pp_s + geom_line(color="#0078b5", size = 0.25) + ylim(0, 1)
l1_pp_s

l1_ss_s <- base_ss_s + geom_line(color="#0078b5", size = 0.25) + ylim(-0.05, 0.05)
l1_ss_s
